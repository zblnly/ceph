...
/**
 * The StateTrackerImpl allows you to dynamically allocate, name, and reference
 * state pointers in a system. Each state pointer consists of:
 * 1) A system id, (generated by the StateTrackerImpl)
...
class StateTrackerImpl {
  /**
   * @defgroup State Single state struct
   * @{
   */
public:
  /**
  ...
  struct State {
    ...
    StateTrackerImpl *system_pointer;
  ...
  private:
    friend class StateTrackerImpl;
    State(StateTrackerImpl *sys, int id, std::string &name) :
  ...
  /**
   * @defgroup StateTrackerImpl object information
   * @{
   */
private:
  /// my system name
  ...
  mutable Mutex lock;
  /**
   * Construct a new StateTrackerImpl.
   *
   * @param name The name of the StateTrackerImpl.
   */
  StateTrackerImpl(const char *name) :
    system_name(name),
    state_id_alloc(0),
    state_names(),
    states(),
...
public:
   ...
   *
   * @param name The name of the StateTracker.
   * @return A StateTracker.
   */
  static std::tr1::shared_ptr<StateTrackerImpl>
    create_state_tracker(const char *name) {
    std::tr1::shared_ptr<StateTrackerImpl> tracker(new StateTrackerImpl(name));
    return tracker;
  }

  /**
   * Retrieve the system name for this StateTrackerImpl.
   * @return the System name, as a string.
   */
  const std::string get_system_name() const { return system_name; }
  /// @} StateTrackerImpl object information
...
public:
  ...
  /**
   * Check if a state belongs to this StateTracker.
   *
...
typedef std::tr1::shared_ptr<StateTrackerImpl> StateTracker;
